...
- **MCC Code & Policy Management**: Complete admin UI for managing Merchant Category Codes (MCC) and their underwriting policies.
  - **MCC Codes Management** (`/mcc-codes`): Full CRUD for the master MCC codes lookup table. Features include search, category filtering, risk level filtering, and inline status toggling. Admin/super_admin can create, edit, and delete codes; underwriters can view. Each code has: 4-digit code, description, category, risk level (low/medium/high), and active status.
  - **MCC Policies Management** (`/mcc-policies`): Configure acquirer-specific underwriting rules for MCC codes. Supports policy types (allowed, requires_review, high_risk, prohibited), risk level overrides, and per-acquirer configurations. Accessible to admin, super_admin, and underwriter roles.
