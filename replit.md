...
- **PDF Field Mapping Phase 2**: Full mapping infrastructure between PDF imported fields and template fields. `pdfMappingConfiguration` stores raw PDF fields with `mappedToTemplateField` and `mappingStatus` (auto/manual/unmapped). GET/PUT endpoints at `/api/acquirer-application-templates/:id/field-mapping` for viewing and modifying mappings. `PdfFieldMappingView` component in template viewer shows mapped/unmapped/no-PDF-link fields with color-coded tables. `pdfGenerator.generateMappedApplicationPDF()` generates PDFs using template field mapping. Export endpoint at `/api/prospect-applications/:id/mapped-pdf`.
