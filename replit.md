...
- **PDF Field Mapping & Filled PDF Generation**: Original uploaded PDFs are now stored as base64 in `original_pdf_base64` column on `acquirer_application_templates`. When a prospect completes an application, the system uses `pdf-lib` to open the original PDF and fill in form fields using the pdfFieldId mapping. If the PDF has no fillable form fields, data is appended as text overlay pages. Key endpoints: GET/PUT `/api/acquirer-application-templates/:id/field-mapping` for mapping management, POST `/:id/upload-pdf` for re-uploading original PDFs to existing templates, GET `/api/prospect-applications/:id/mapped-pdf` for generating filled PDFs. Frontend shows `PdfFieldMappingView` with color-coded mapped/unmapped tables and `UploadOriginalPdf` button in template viewer.
