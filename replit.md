...
- **Epic D — Campaign Linkage & Auto-Assignment (rev 1)**: Per Charrg FRD, four linkage paths land an MPA on the right pricing campaign:
  - **Schema**: `agents.defaultCampaignId` (integer) + new `campaign_assignment_rules` table (mcc/acquirerId/agentId wildcards → campaignId, priority, isActive, notes, createdBy). Insert schemas + types in `shared/schema.ts`.
  - **Storage** (`server/storage.ts`): `swapCampaignForProspect` (deactivates prior active assignments, inserts new — keeps history), `getProspectsForCampaign` (active assignments only), 5 rule CRUD methods incl. `findCampaignByRule` (most-specific match wins, tie-break by priority asc → id asc).
  - **Routes** (`server/routes.ts`): `GET/POST/PATCH/DELETE /api/campaign-rules`, `POST /api/prospects/:id/set-campaign` (with `regenerate?: boolean`), `GET /api/campaigns/:id/affected-applications`, `POST /api/campaigns/:id/regenerate-pdfs`. `POST /api/prospects` fallback chain: explicit `campaignId` → agent.`defaultCampaignId` → rules engine → 400.
  - **UI**: `agent-modal.tsx` adds Default Campaign select; `prospects.tsx` auto-pre-selects campaign when agent changes; `campaign-view.tsx` adds "Copy Prefilled MPA Link" + "Open Prefilled MPA" using `?campaignId=`; `application-view.tsx` adds "Set Campaign" dialog with regenerate-PDF checkbox; `merchant-application.tsx` honors `?campaignId=` and `?agentId=`; `enhanced-campaign-dialog.tsx` after-PUT prompt offers bulk regenerate when affected count > 0; new `/campaign-rules` page with full CRUD; sidebar Campaigns sub-nav now lists "Assignment Rules". TS error count: 612 (baseline 609; +3 cascade in agent-modal from new field, no new errors in any of the new files).
